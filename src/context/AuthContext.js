...
export function AuthProvider({ children }) {
  ...
  // Super Admin & Admin: Add Bus
  const addBus = async (busNumber) => {
    if (user?.role !== 'super_admin' && user?.role !== 'admin') {
      throw new Error('Only Admins or Super Admin can add buses');
    }
    const newBus = {
      id: Date.now().toString(),
      number: busNumber,
      status: 'active',
      currentLocation: { lat: 11.2760, lng: 77.5932 },
    };
  ...
  // Super Admin & Admin: Add Driver
  const addDriver = async (name, email, password, phone) => {
    if (user?.role !== 'super_admin' && user?.role !== 'admin') {
      throw new Error('Only Admins or Super Admin can add drivers');
    }
    const newDriver = {
      id: Date.now().toString(),
      name,
  ...
  // Assign Driver to Bus (Daily)
  const assignDriverToBus = async (driverId, busId) => {
    if (user?.role !== 'super_admin' && user?.role !== 'admin') {
      throw new Error('Only Admins or Super Admin can assign drivers');
    }
